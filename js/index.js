...
var particles = [];
var particlesFired = 0;

var statusSpeech = document.getElementById('status_speech');
var statusTought = document.getElementById('status_tought');

var spinButton = document.getElementById('rodar');
...
window.onload = function() {
    ...
    statusSpeech.innerHTML = 'Spin and win prizes!';
};
...
function checkStartDrag(e) {
    ...
    if (wheelSpinning === true) {
        wheelSpinning = false;
        wheelStopped = true;
        // statusSpeech.innerHTML = "Impatience will not be rewarded.";
    }
}
...
function initPhysics() {
    ...
    wheel = new Wheel(wheelX, wheelY, wheelRadius, 22, 0.25, 7.5);
...
function update() {
    ...
    var currentPosition = wheel.currentPosition();
    var fixedCurrentPos = (currentPosition < 5) ? currentPosition + 15 : currentPosition - 5;

    var velocity = wheel.body.angularVelocity;

    if(Math.abs(velocity) > 1) {
      statusSpeech.style.display = "none";
      statusTought.style.display = "block";
      particlesFired = 0;
      timer = 0;
    } else {
      statusTought.style.display = "none";
      statusSpeech.innerHTML = 'And the prize is ...';
      statusSpeech.style.display = "block";
      ...
      if (Math.abs(velocity) < 0.04) {
        ...
        if ($('#'+prizes_pos[fixedCurrentPos].prize).hasClass('disable')) {
          statusSpeech.innerHTML = 'ohhhh ... Try again!';
        } else {
          statusSpeech.innerHTML = 'ESPETAAAACULO!';
          if (particlesFired == 0){
            spawnPartices();
            particlesFired = 1;
          }
        ...
        if(timer > 300) statusSpeech.innerHTML = 'Spin and win prizes!';
      }
    }
}
...
Wheel.prototype = {
    ...
    draw:function() {
        ...
        ctx.beginPath();
        ctx.fillStyle = '#048ae6';
        ctx.arc(0, 0, this.pRadius + 19, 0, TWO_PI);
        ctx.fill();

        ctx.rotate(-this.body.angle);

        for (var i = 0; i < this.segments; i++) {
          ctx.fillStyle = prizes_pos[i].colour;
...
spinButton.onclick = function() {
  wheel.body.angularVelocity = Math.floor((Math.random() * 20) + 8);
  wheelSpinning = true;
  wheelStopped = false;
};

var confirmPrize = function() {
  $( function() {
...
var submitPrize = function() {
  var prize = $('#confirmBox').val();

  $.post( "php/actions.php", { prize: prize }).done(function( data ) {
    checkPrizeAvailability(prize);
    $("#dialog-confirm").dialog( "close" );
  });
}

var checkPrizeAvailability = function(prize) {
  prizes_track[prize].times_given++;

  if (prizes_track[prize].times_given >= prizes_track[prize].quantity) { disablePrize(prize); }
}

var disablePrize = function(prize) {
  $('#'+prize).removeClass('enable').addClass('disable');
}